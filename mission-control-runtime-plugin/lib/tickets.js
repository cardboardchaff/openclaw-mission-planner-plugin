  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      ...
      while (Date.now() < waitUntil) {
        // busy-wait short delay in sync call path
      }
    }
  }
...
function requireStatus(value, field) {
  const wanted = requireText(value, field);
  if (!ALLOWED_STATUSES.has(wanted)) {
    throw new Error(`invalid ${field}: ${wanted}. allowed=${Array.from(ALLOWED_STATUSES).join(",")}`);
  }
  return wanted;
}
...
function requireDeliveryState(value, field) {
  const wanted = requireText(value, field);
  if (!ALLOWED_DELIVERY_STATES.has(wanted)) {
    throw new Error(`invalid ${field}: ${wanted}. allowed=${Array.from(ALLOWED_DELIVERY_STATES).join(",")}`);
  }
  return wanted;
}
